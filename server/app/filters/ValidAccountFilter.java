...
public class ValidAccountFilter extends EssentialFilter {
  ...
  public EssentialAction apply(EssentialAction next) {
    return EssentialAction.of(
        request -> {
          ...
          boolean isValidExtendSessionRequest =
              logoutReason.isPresent()
                  && logoutReason.get().equals(LogoutReason.INVALID_SESSION)
                  && isExtendSessionRequest(request.uri());
          if (logoutReason.isPresent()
              && !allowedEndpoint(request)
              && !isValidExtendSessionRequest) {
          ...
          profile.ifPresent(p -> p.getProfileData().updateLastActivityTime());
  ...
  private Optional<LogoutReason> getLogoutReason(Optional<CiviFormProfile> maybeProfile) {
    ...
    long lastActivityTime = profile.getProfileData().getLastActivityTime();
    long sessionStartTime = profile.getProfileData().getSessionStartTime();
  ...
  private boolean isExtendSessionRequest(String uri) {
    return uri.startsWith("/extend-session");
  }
}
